...
\begin{itemize}
\item generuje náhodně polygon s daným počtem bodů, zatím pouze jeden
\item body polygonu jsem generovány v rozsahu 100 až 700 pro x-ovou souřadnici a 50 až 650 pro y-ovou souřadnici
\item vstupem je uživatelem zadaný počet bodů přes widget lineEdit
\end{itemize}
\item[] \underline{GrahamScan}
\begin{itemize}
\item opravuje pořadí vstupních bodů podle velikosti úhlu omega tak, aby bylo možné sestavit nekonvexní polygon s využitím algoritmu Graham Scan pro tvorbu nekonvexní obálky.
\end{itemize}
\end{enumerate}
...
\begin{itemize}
\item přistupuje k členské proměnné generated\_points a vkládá do ní vygenerované a Graham Scan algoritmem zpracované (seřazené) body.
\end{itemize}
\end{enumerate}
...
\begin{itemize}
\item Třídí body ve vektoru podle velikosti y-ové souřadnice každého z bodů. 
\end{itemize}
\subsection{Trieda SortByOmega}
\begin{itemize}
\item Třídí body ve vektoru podle velikosti úhlu omega. Tedy od rovnoběžky daným bodem, nejčastastěji pivotem s minimální nebo maximální jednou ze souřadnic. V našem případě od bodu s nejmenší y-ovou souřadnicí.
\end{itemize}
%-------------------------------------------------------------------------
